...
export function App() {
  ...
  return (
    <div class="flex flex-col h-full min-h-0 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 transition-colors duration-150">
      {/* Header */}
      <Header />
      ...
      <div class="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div class="flex flex-1 min-w-0 flex-col md:flex-row">
          {/* Input panel */}
          <div class="flex-1 min-h-0 min-w-0 flex flex-col border-b md:border-b-0 md:border-r border-zinc-200 dark:border-zinc-800 md:max-h-full"
        ...
        <Show when={store.sidebarOpen}>
          <div class="lg:hidden fixed inset-0 z-40 flex" onClick={() => setStore('sidebarOpen', false)}>
            ...
            <div class="w-72 flex flex-col bg-white dark:bg-zinc-950 border-l border-zinc-200 dark:border-zinc-800 h-full overflow-hidden">
              <DetectionSidebar />
            </div>
          </div>
        </Show>
      ...
      {/* Footer */}
      <footer class="flex items-center justify-between px-4 py-2 border-t border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 shrink-0">
        <Disclaimer />

        {/* Sidebar toggle for desktop */}
        <button
          onClick={() => setStore('sidebarOpen', (v) => !v)}
          class="hidden lg:flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors"
          title={t('detection.toggleSidebar')}
        >
          <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
          </svg>
          {store.sidebarOpen ? t('detection.hide') : t('detection.title')}
        </button>
      </footer>
    </div>
  );
